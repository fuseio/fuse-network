...
contract("ProxyStorage", async (accounts) => {
  ...
  describe("initializeAddresses", async () => {
    ...
    it("should fail if not called from owner", async () => {
      await proxyStorage
        .initializeAddresses(blockReward.address, voting.address, {
          from: nonOwner,
        })
  ...
  describe("upgradeTo", async () => {
    ...
    it("should only be called by ProxyStorage (this)", async () => {
      await proxy
        .upgradeTo(proxyStorageNew.address, { from: owner })
        .should.be.rejectedWith(ERROR_MSG);
      let { logs } = await proxy.upgradeTo(proxyStorageNew.address, {
    ...
    it("should change implementation address", async () => {
      await proxy.upgradeTo(proxyStorageNew.address, {
        from: proxyStorageStub,
      });
      await proxy.setProxyStorageMock(proxyStorage.address);
    ...
    it("should increment implementation version", async () => {
      ...
      await proxy.setProxyStorageMock(proxyStorage.address);
      proxyStorageNewVersion.should.be.bignumber.equal(
        await proxy.getVersion()
      );
    });
