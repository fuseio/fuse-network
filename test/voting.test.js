...
contract("Voting", async (accounts) => {
  ...
  describe("initialize", async () => {
    it("should be successful", async () => {
      ...
      toBN(MIN_BALLOT_DURATION_CYCLES).should.be.bignumber.equal(
        await voting.getMinBallotDurationCycles()
      );
    });
  });
  ...
  describe("newBallot", async () => {
    ...
    it("should be successful", async () => {
      ...
      let voteStartAfterNumberOfBlocks = toBN(voteStartAfterNumberOfCycles).mul(
        toBN(CYCLE_DURATION_BLOCKS)
      );
      let startBlock = currentCycleEndBlock.add(voteStartAfterNumberOfBlocks);
      let voteEndAfterNumberOfBlocks = toBN(voteCyclesDuration).mul(
        toBN(CYCLE_DURATION_BLOCKS)
      );
      ...
      toBN(QUORUM_STATES.IN_PROGRESS).should.be.bignumber.equal(
    ...
    it("should fail if duration is invalid", async () => {
      ...
      await voting
        .newBallot(
          voteStartAfterNumberOfCycles,
          minBallotDurationCycles.sub(toBN(1)),
          contractType,
          proposedValue,
          "description",
          { from: validators[0] }
      ...
      await voting
        .newBallot(
          voteStartAfterNumberOfCycles,
          maxBallotDurationCycles.add(toBN(1)),
          contractType,
          proposedValue,
          "description",
          { from: validators[0] }
    ...
    it("should fail if creating ballot over the ballots limit", async () => {
      ...
      let ballotLimitPerValidator = (
        await voting.getBallotLimitPerValidator()
      ).toNumber();
      ballotLimitPerValidator.should.be.equal(
        Math.floor(maxLimitOfBallots / validatorsCount)
      );
  ...
  describe("vote", async () => {
    ...
    it('should vote "accept" successfully', async () => {
      ...
      let { logs } = await voting.vote(id, ACTION_CHOICES.ACCEPT, {
        from: validators[0],
      }).should.be.fulfilled;
      logs.length.should.be.equal(1);
      logs[0].event.should.be.equal("Vote");
      logs[0].args["id"].should.be.bignumber.equal(id);
      logs[0].args["decision"].should.be.bignumber.equal(
        toBN(ACTION_CHOICES.ACCEPT)
      );
      logs[0].args["voter"].should.be.equal(validators[0]);
      toBN(ACTION_CHOICES.ACCEPT).should.be.bignumber.equal(
        await voting.getVoterChoice(id, validators[0])
      );
    });
    it('should vote "reject" successfully', async () => {
      ...
      let blocksToAdvance = voteStartBlock.sub(currentBlock);
      await advanceBlocks(blocksToAdvance.toNumber());
      let { logs } = await voting.vote(id, ACTION_CHOICES.REJECT, {
        from: validators[0],
      }).should.be.fulfilled;
      logs.length.should.be.equal(1);
      logs[0].event.should.be.equal("Vote");
      logs[0].args["id"].should.be.bignumber.equal(id);
      logs[0].args["decision"].should.be.bignumber.equal(
        toBN(ACTION_CHOICES.REJECT)
      );
      logs[0].args["voter"].should.be.equal(validators[0]);
      toBN(ACTION_CHOICES.REJECT).should.be.bignumber.equal(
        await voting.getVoterChoice(id, validators[0])
      );
    });
    it("multiple voters should vote successfully", async () => {
      ...
      let blocksToAdvance = voteStartBlock.sub(currentBlock);
      await advanceBlocks(blocksToAdvance.toNumber());
      await voting.vote(id, ACTION_CHOICES.ACCEPT, { from: validators[0] })
        .should.be.fulfilled;
      toBN(ACTION_CHOICES.ACCEPT).should.be.bignumber.equal(
        await voting.getVoterChoice(id, validators[0])
      );

      await voting.vote(id, ACTION_CHOICES.ACCEPT, { from: validators[1] })
        .should.be.fulfilled;
      toBN(ACTION_CHOICES.ACCEPT).should.be.bignumber.equal(
        await voting.getVoterChoice(id, validators[1])
      );

      await voting.vote(id, ACTION_CHOICES.REJECT, { from: validators[2] })
        .should.be.fulfilled;
      toBN(ACTION_CHOICES.REJECT).should.be.bignumber.equal(
        await voting.getVoterChoice(id, validators[2])
      );

      await voting.vote(id, ACTION_CHOICES.REJECT, { from: validators[3] })
        .should.be.fulfilled;
      toBN(ACTION_CHOICES.REJECT).should.be.bignumber.equal(
        await voting.getVoterChoice(id, validators[3])
      );
    });
    it("should be successful even if called by non validator", async () => {
      ...
      let blocksToAdvance = voteStartBlock.sub(currentBlock);
      await advanceBlocks(blocksToAdvance.toNumber());
      await voting.vote(id, ACTION_CHOICES.ACCEPT, { from: nonValidatorKey })
        .should.be.fulfilled;
      toBN(ACTION_CHOICES.ACCEPT).should.be.bignumber.equal(
        await voting.getVoterChoice(id, nonValidatorKey)
      );
    });
    it("should fail if voting before start time", async () => {
      ...
      await advanceBlocks(blocksToAdvance.toNumber());
      await voting
        .vote(id, ACTION_CHOICES.ACCEPT, { from: validators[0] })
        .should.be.rejectedWith(ERROR_MSG);
    });
    ...
    it("should fail if trying to vote with invalid choice", async () => {
      ...
      await advanceBlocks(blocksToAdvance.toNumber());
      await voting
        .vote(id, ACTION_CHOICES.INVALID, { from: validators[0] })
        .should.be.rejectedWith(ERROR_MSG);
      await voting
        .vote(id, Object.keys(ACTION_CHOICES).length + 1, {
          from: validators[0],
        })
        .should.be.rejectedWith(ERROR_MSG);
    });
    it("should fail if trying to vote for invalid id", async () => {
      ...
      await advanceBlocks(blocksToAdvance.toNumber());
      await voting
        .vote(id.toNumber() + 1, ACTION_CHOICES.ACCEPT, { from: validators[0] })
        .should.be.rejectedWith(ERROR_MSG);
    });
  });
  ...
  describe("onCycleEnd", async () => {
    ...
    it("should reject voting that got majority but does not pass the turnout", async () => {
      ...
      await advanceBlocks(
        voteEndBlock.sub(currentBlock).add(toBN(1)).toNumber()
      );
    ...
    it("should reject a voting that do not got majority and pass the turnout", async () => {
      ...
      await voting.vote(id, ACTION_CHOICES.REJECT, { from: validators[0] })
        .should.be.fulfilled;
      await voting.vote(id, ACTION_CHOICES.REJECT, { from: validators[1] })
        .should.be.fulfilled;
      await voting.vote(id, ACTION_CHOICES.REJECT, { from: validators[2] })
        .should.be.fulfilled;
      await voting.vote(id, ACTION_CHOICES.ACCEPT, { from: validators[3] })
        .should.be.fulfilled;
      await voting.vote(id, ACTION_CHOICES.ACCEPT, { from: validators[4] })
        .should.be.fulfilled;
    ...
    it("should approve if number of votes for is less than against but has a higher voting power", async () => {
      ...
      await consensus.addStakeAmountMock(
        validators[0],
      ...
      await voting.vote(id, ACTION_CHOICES.REJECT, { from: validators[3] })
        .should.be.fulfilled;
      await voting.vote(id, ACTION_CHOICES.REJECT, { from: validators[4] })
        .should.be.fulfilled;
    ...
    it("should reject if number of votes against is less than for but has a higher voting power", async () => {
      ...
      await voting.vote(id, ACTION_CHOICES.REJECT, { from: validators[1] })
        .should.be.fulfilled;
      await voting.vote(id, ACTION_CHOICES.ACCEPT, { from: validators[2] })
        .should.be.fulfilled;
      await voting.vote(id, ACTION_CHOICES.ACCEPT, { from: validators[3] })
        .should.be.fulfilled;
      await voting.vote(id, ACTION_CHOICES.ACCEPT, { from: validators[4] })
    ...
    it("golden flow should work", async () => {
      ...
      await voting.newBallot(
        voteStartAfterNumberOfCycles * 2,
        voteCyclesDuration,
        contractType,
        proposedValue,
        "description",
      ...
      expected.first.accepted.should.be.bignumber.equal(
        await voting.getAccepted(firstBallotId)
      );
      expected.first.rejected.should.be.bignumber.equal(
        await voting.getRejected(firstBallotId)
      );
      expected.second.accepted.should.be.bignumber.equal(
        await voting.getAccepted(secondBallotId)
      );
      expected.second.rejected.should.be.bignumber.equal(
        await voting.getRejected(secondBallotId)
      );
      expected.third.accepted.should.be.bignumber.equal(
        await voting.getAccepted(thirdBallotId)
      );
      expected.third.rejected.should.be.bignumber.equal(
        await voting.getRejected(thirdBallotId)
      );

      // vote on 1st ballot
      await voting.vote(firstBallotId, ACTION_CHOICES.ACCEPT, {
        from: validators[0],
      }).should.be.fulfilled;
      await voting.vote(firstBallotId, ACTION_CHOICES.REJECT, {
        from: validators[1],
      }).should.be.fulfilled;
      await voting.vote(firstBallotId, ACTION_CHOICES.ACCEPT, {
        from: validators[2],
      ...
      totals.first.accepted = totals.first.accepted.add(
        val0stake.add(val2stake)
      );
      totals.first.rejected = totals.first.rejected.add(val1stake);

      // vote on 2nd ballot
      await voting.vote(secondBallotId, ACTION_CHOICES.REJECT, {
        from: validators[0],
      }).should.be.fulfilled;
      await voting.vote(secondBallotId, ACTION_CHOICES.ACCEPT, {
        from: validators[1],
      }).should.be.fulfilled;
      await voting.vote(secondBallotId, ACTION_CHOICES.REJECT, {
        from: validators[2],
      ...
      totals.second.rejected = totals.second.rejected.add(
        val0stake.add(val2stake)
      );
      ...
      totals.first.accepted = totals.first.accepted.add(
        val3stake.add(val5stake.add(val6stake))
      );
      ...
      // vote on 3rd ballot
      await voting.vote(thirdBallotId, ACTION_CHOICES.ACCEPT, {
        from: validators[0],
      }).should.be.fulfilled;
      await voting.vote(thirdBallotId, ACTION_CHOICES.ACCEPT, {
        from: validators[1],
      ...
      await voting.vote(thirdBallotId, ACTION_CHOICES.REJECT, {
        from: nonValidatorKey,
      }).should.be.fulfilled;

      totals.third.accepted = totals.third.accepted.add(
        val0stake.add(val1stake.add(val2stake))
      );
      ...
      // vote on 3rd ballot
      await voting
        .vote(thirdBallotId, ACTION_CHOICES.ACCEPT, { from: validators[0] })
        .should.be.rejectedWith(ERROR_MSG);
      await voting.vote(thirdBallotId, ACTION_CHOICES.ACCEPT, {
        from: validators[3],
      }).should.be.fulfilled;
