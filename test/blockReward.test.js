...
contract.only("BlockReward", async (accounts) => {
  ...
  describe("initialize", async () => {
    it("default values", async () => {
      ...
      toChecksumAddress(SYSTEM_ADDRESS).should.be.equal(
        toChecksumAddress(await blockReward.getSystemAddress())
      );
      ...
      blockRewardAmount.should.be.bignumber.equal(
        await blockReward.getBlockRewardAmount()
      );
    });
  });
  ...
  describe("reward", async () => {
    ...
    describe("#getBlockRewardAmountPerValidator", () => {
      ...
      it("block reward with one validator", async () => {
        ...
        const blockRewardAmountOfV =
          await blockReward.getBlockRewardAmountPerValidator(validator);
      ...
      it("block reward does not change if the propotion stays the same", async () => {
        ...
        await consensus.sendTransaction({
          from: validator,
          value: minStakeAmount.mul(toBN(4)),
        }).should.be.fulfilled;
      ...
      it("block reward for two validators", async () => {
        ...
        await consensus.sendTransaction({
          ...
          value: minStakeAmount.mul(THREE),
        }).should.be.fulfilled;

        await consensus.setTotalStakeAmountMock(minStakeAmount.mul(FOUR));
        await consensus.setCurrentValidatorsLengthMock(TWO);

        let blockRewardAmountOfV =
          await blockReward.getBlockRewardAmountPerValidator(validator);
        let expectedReward = blockRewardAmount.div(FOUR).mul(TWO);
        expectedReward.should.be.bignumber.equal(blockRewardAmountOfV);

        blockRewardAmountOfV =
          await blockReward.getBlockRewardAmountPerValidator(secondValidator);
    ...
    it("can only be called by system address", async () => {
      ...
      await blockReward.setSystemAddressMock(mockSystemAddress, {
        from: owner,
      });
      await consensus.sendTransaction({ from: owner, value: minStakeAmount })
        .should.be.fulfilled;
      await blockReward.reward([accounts[3]], [0], { from: mockSystemAddress })
        .should.be.fulfilled;
    });
    it("should revert if input array contains more than one item", async () => {
      ...
      await blockReward
        .reward([accounts[3], accounts[4]], [0, 0], { from: mockSystemAddress })
        .should.be.rejectedWith(ERROR_MSG);
    });
    it("should revert if lengths of input arrays are not equal", async () => {
      ...
      await blockReward
        .reward([accounts[3]], [0, 0], { from: mockSystemAddress })
        .should.be.rejectedWith(ERROR_MSG);
    });
    it('should revert if "kind" parameter is not 0', async () => {
      ...
      await blockReward
        .reward([accounts[3]], [1], { from: mockSystemAddress })
        .should.be.rejectedWith(ERROR_MSG);
    });
    it("should give reward to validator and total supply should be updated", async () => {
      ...
      let blockRewardAmount =
        await blockReward.getBlockRewardAmountPerValidator(validator);
      let { logs } = await blockReward.reward([validator], [0], {
        from: mockSystemAddress,
      }).should.be.fulfilled;
      ...
      expectedSupply.should.be.bignumber.equal(
    ...
    it("should give rewards to validator and its delegators", async () => {
      ...
      let delegateAmountValue = parseInt(
        (minStakeAmount.div(decimals).toNumber() * 0.99) / delegatorsCount
      );
      let delegateAmount = toWei(toBN(delegateAmountValue), "ether");
      let stakeAmountValue =
        minStakeAmount.div(decimals).toNumber() -
        delegateAmountValue * delegatorsCount;
      ...
      await consensus.setValidatorFeeMock(fee, { from: validator }).should.be
        .fulfilled;
      ...
      let expectedRewardForDelegators = blockRewardAmount
        .mul(delegateAmount)
        .div(minStakeAmount)
      ...
      for (let i = 1; i <= delegatorsCount; i++) {
        ...
        expectedRewardForValidator = expectedRewardForValidator.sub(
          expectedRewardForDelegators
        );
      }
    ...
    it("reward amount should update after BLOCKS_PER_YEAR and total yearly inflation should be calculated correctly", async () => {
      ...
      newBlockRewardAmount.should.be.bignumber.equal(
        totalSupply
          .mul(decimals)
          .mul(inflation)
          .div(toBN(10000))
  ...
  describe("emitRewardedOnCycle", function () {
    ...
    it("should fail if not called by validator", async () => {
      await blockReward
        .emitRewardedOnCycle({ from: nonOwner })
        .should.be.rejectedWith(ERROR_MSG);
    });
    it("should be successful if `shouldEmitRewardedOnCycle` and `consensus.isFinalized` are true", async () => {
      ...
      await blockReward
        .emitRewardedOnCycle({ from: owner })
        .should.be.rejectedWith(ERROR_MSG);
      ...
      await blockReward.setShouldEmitRewardedOnCycleMock(true);
      await consensus.setFinalizedMock(true);
      await blockReward.emitRewardedOnCycle({ from: owner }).should.be
        .fulfilled;
    });
    it("should be successful and emit event", async () => {
      ...
      await blockReward.setShouldEmitRewardedOnCycleMock(true);
      let { logs } = await blockReward.emitRewardedOnCycle({ from: owner })
        .should.be.fulfilled;
  ...
  describe("upgradeTo", async () => {
    ...
    it("should only be called by ProxyStorage", async () => {
      ...
      await proxy
        .upgradeTo(blockRewardNew.address, { from: owner })
        .should.be.rejectedWith(ERROR_MSG);
      let { logs } = await proxy.upgradeTo(blockRewardNew.address, {
    ...
    it("should change implementation address", async () => {
      blockRewardOldImplementation.should.be.equal(
