...
contract("Consensus", async (accounts) => {
  ...
  describe("initialize", async () => {
    it("default values", async () => {
      ...
      toChecksumAddress(SYSTEM_ADDRESS).should.be.equal(
        toChecksumAddress(await consensus.getSystemAddress())
      );
      ...
      toBN(MAX_VALIDATORS).should.be.bignumber.equal(
        await consensus.getMaxValidators()
      );
      toBN(CYCLE_DURATION_BLOCKS).should.be.bignumber.equal(
        await consensus.getCycleDurationBlocks()
      );
      toBN(SNAPSHOTS_PER_CYCLE).should.be.bignumber.equal(
        await consensus.getSnapshotsPerCycle()
      );
      ZERO.should.be.bignumber.equal(
        await consensus.stakeAmount(initialValidator)
      );
      ZERO.should.be.bignumber.equal(await consensus.totalStakeAmount());
      false.should.be.equal(await consensus.hasCycleEnded());
      ZERO.should.be.bignumber.equal(
        await consensus.getLastSnapshotTakenAtBlock()
      );
    ...
    it("initial validator is added to pending after sending fuse", async () => {
      ...
      toBN(MIN_STAKE).should.be.bignumber.equal(
        await consensus.stakeAmount(initialValidator)
      );
      toBN(MIN_STAKE).should.be.bignumber.equal(
        await consensus.totalStakeAmount()
      );
    });
  });
  ...
  describe("setProxyStorage", async () => {
    ...
    it("setProxyStorage should fail if no address", async () => {
      await consensus
        .setProxyStorage(ZERO_ADDRESS)
        .should.be.rejectedWith(ERROR_MSG);
    });

    it("setProxyStorage should fail if not called by owner", async () => {
      await consensus
        .setProxyStorage(proxyStorage.address, { from: nonOwner })
        .should.be.rejectedWith(ERROR_MSG);
      // success
      ...
      await consensus
        .setProxyStorage(RANDOM_ADDRESS)
        .should.be.rejectedWith(ERROR_MSG);
    });
  ...
  describe("emitInitiateChange", async () => {
    ...
    it("should fail if not called by validator", async () => {
      await consensus
        .emitInitiateChange({ from: nonOwner })
        .should.be.rejectedWith(ERROR_MSG);
    });
    ...
    it("should fail if newValidatorSet is empty", async () => {
      ...
      await consensus
        .emitInitiateChange({ from: initialValidator })
        .should.be.rejectedWith(ERROR_MSG);
    });
    ...
    it("should be successful and emit event", async () => {
      ...
      let mockSet = [firstCandidate, secondCandidate];
      await consensus.setNewValidatorSetMock(mockSet);
      let { logs } = await consensus.emitInitiateChange({
        from: initialValidator,
      }).should.be.fulfilled;
  ...
  describe("finalizeChange", async () => {
    ...
    context("with staking", () => {
      it("adding validators should update the total stake", async () => {
        ...
        MIN_STAKE.mul(TWO).should.be.bignumber.equal(
          await consensus.totalStakeAmount()
        );
      });
  ...
  describe("stake (fallback function)", async () => {
    ...
    describe("basic", async () => {
      it("should not allow zero stake", async () => {
        await consensus
          .send(0, { from: firstCandidate })
          .should.be.rejectedWith(ERROR_MSG);
      });

      context("with the initial validator", () => {
        const validator = initialValidator;
        it("less than minimum stake, should update the total stake", async () => {
          await consensus.sendTransaction({
            from: validator,
            value: LESS_THAN_MIN_STAKE,
          }).should.be.fulfilled;
          // contract balance should be updated
          LESS_THAN_MIN_STAKE.should.be.bignumber.equal(
            await web3.eth.getBalance(consensus.address)
          );
          // sender stake amount should be updated
          LESS_THAN_MIN_STAKE.should.be.bignumber.equal(
            await consensus.stakeAmount(validator)
          );
          ...
          ZERO.should.be.bignumber.equal(
            await consensus.validatorFee(validator)
          );
        });

        it("minimum stake amount", async () => {
          await consensus.sendTransaction({ from: validator, value: MIN_STAKE })
            .should.be.fulfilled;
          MIN_STAKE.should.be.bignumber.equal(
            await web3.eth.getBalance(consensus.address)
          );
          MIN_STAKE.should.be.bignumber.equal(
            await consensus.stakeAmount(validator)
          ...
          defaultValidatorFee.should.be.bignumber.equal(
            await consensus.validatorFee(validator)
          );
        });
        ...
        it("should allow more than minimum stake", async () => {
          await consensus.sendTransaction({
            from: validator,
            value: MORE_THAN_MIN_STAKE,
          }).should.be.fulfilled;
          MORE_THAN_MIN_STAKE.should.be.bignumber.equal(
            await web3.eth.getBalance(consensus.address)
          );
          MORE_THAN_MIN_STAKE.should.be.bignumber.equal(
            await consensus.stakeAmount(validator)
        ...
        it("should allow the maximum stake", async () => {
          ...
          await consensus.sendTransaction({ from: validator, value: MAX_STAKE })
            .should.be.fulfilled;
          MAX_STAKE.should.be.bignumber.equal(
            await web3.eth.getBalance(consensus.address)
          );
          MAX_STAKE.should.be.bignumber.equal(
            await consensus.stakeAmount(validator)
        ...
        it("should not allow more the maximum stake", async () => {
          await consensus
            .sendTransaction({ from: validator, value: MORE_THAN_MAX_STAKE })
            .should.be.rejectedWith(ERROR_MSG);
        });
      });

      context("with first candidate", () => {
        ...
        it("minimum stake amount", async () => {
          let validatorsLength = await consensus.currentValidatorsLength();
          validatorsLength.should.be.bignumber.equal(ONE);
          true.should.be.equal(await consensus.isValidator(initialValidator));
          ...
          // total stake is updated on the EoC
          await mockEoC();
          MIN_STAKE.should.be.bignumber.equal(
            await consensus.totalStakeAmount()
          );
          ...
          validatorsLength.should.be.bignumber.equal(ONE);
          true.should.be.equal(await consensus.isValidator(validator));
        });
    ...
    describe("advanced", async () => {
      context("with first candidate", () => {
        it("minimum stake amount, in more than one transaction", async () => {
          ...
          LESS_THAN_MIN_STAKE.should.be.bignumber.equal(
            await consensus.stakeAmount(firstCandidate)
          );
          ZERO.should.be.bignumber.equal(await consensus.totalStakeAmount());
        ...
        it("multiple validators, multiple times", async () => {
          ...
          pendingValidators.should.deep.equal(expectedValidators);
          MIN_STAKE.should.be.bignumber.equal(
            await consensus.stakeAmount(secondCandidate)
          );
          ZERO.mul(TWO).should.be.bignumber.equal(
            await consensus.totalStakeAmount()
          );
          ...
          MIN_STAKE.mul(THREE).should.be.bignumber.equal(
            await consensus.totalStakeAmount()
          );
          ...
          MIN_STAKE.mul(FOUR).should.be.bignumber.equal(
            await consensus.totalStakeAmount()
          );

          // total stake does not change of the EoC
          await mockEoC();
          MIN_STAKE.mul(FOUR).should.be.bignumber.equal(
            await consensus.totalStakeAmount()
          );
      ...
      context("with existing stake", () => {
        ...
        it("minimum stake amount, in more than one transaction", async () => {
          ...
          MIN_STAKE.add(LESS_THAN_MIN_STAKE).should.be.bignumber.equal(
            await web3.eth.getBalance(consensus.address)
          );
          MIN_STAKE.add(LESS_THAN_MIN_STAKE).should.be.bignumber.equal(
            await consensus.stakeAmount(firstCandidate)
        ...
        it("more than minimum stake amount, in more than one transaction", async () => {
          ...
          MIN_STAKE.add(MORE_THAN_MIN_STAKE).should.be.bignumber.equal(
            await web3.eth.getBalance(consensus.address)
          );
          MIN_STAKE.add(MORE_THAN_MIN_STAKE).should.be.bignumber.equal(
            await consensus.stakeAmount(firstCandidate)
        ...
        it("multiple validators, multiple times", async () => {
          ...
          MIN_STAKE.mul(FIVE).should.be.bignumber.equal(
            await consensus.totalStakeAmount()
          );
      ...
      context("total stake", () => {
        it("two validators", async () => {
          ...
          MIN_STAKE.mul(FIVE)
            .add(ONE_ETHER)
            .should.be.bignumber.equal(await consensus.totalStakeAmount());

          await consensus.sendTransaction({
            from: thirdCandidate,
            value: ONE_ETHER,
          }).should.be.fulfilled;
          ONE_ETHER.should.be.bignumber.equal(
            await consensus.stakeAmount(thirdCandidate)
          );
        ...
        it("validator with delegators", async () => {
          await consensus.sendTransaction({
            from: firstCandidate,
            value: MIN_STAKE,
          }).should.be.fulfilled;
          await consensus.delegate(firstCandidate, {
            from: firstDelegator,
            value: MIN_STAKE,
          }).should.be.fulfilled;
          MIN_STAKE.mul(TWO).should.be.bignumber.equal(
          ...
          await consensus.delegate(secondCandidate, {
            from: firstDelegator,
            value: ONE_ETHER,
          }).should.be.fulfilled;
  ...
  describe("stake", async () => {
    ...
    describe("basic", async () => {
      it("should not allow zero stake", async () => {
        await consensus
          .stake({ from: firstCandidate, value: 0 })
          .should.be.rejectedWith(ERROR_MSG);
      });
      it("less than minimum stake - should not be added to pending validators", async () => {
        await consensus.stake({
          from: firstCandidate,
          value: LESS_THAN_MIN_STAKE,
        }).should.be.fulfilled;
        // contract balance should be updated
      ...
      it("minimum stake amount", async () => {
        let pendingValidators = await consensus.pendingValidators();
        pendingValidators.length.should.be.equal(0);
        ONE.should.be.bignumber.equal(
          await consensus.currentValidatorsLength()
        );
        let validators = await consensus.getValidators();
        validators.length.should.be.equal(1);
        validators[0].should.be.equal(initialValidator);
        ...
        await consensus.stake({ from: firstCandidate, value: MIN_STAKE }).should
          .be.fulfilled;
        MIN_STAKE.should.be.bignumber.equal(
          await web3.eth.getBalance(consensus.address)
        );
      ...
      it("should allow more than minimum stake", async () => {
        ...
        let defaultValidatorFee = await consensus.DEFAULT_VALIDATOR_FEE();
        defaultValidatorFee.should.be.bignumber.equal(
          await consensus.validatorFee(firstCandidate)
        );
      ...
      it("should allow the maximum stake", async () => {
        ...
        await consensus.stake({ from: firstCandidate, value: MAX_STAKE }).should
          .be.fulfilled;
        MAX_STAKE.should.be.bignumber.equal(
          await web3.eth.getBalance(consensus.address)
        );
      ...
      it("should not allow more the maximum stake", async () => {
        await consensus
          .stake({ from: firstCandidate, value: MORE_THAN_MAX_STAKE })
          .should.be.rejectedWith(ERROR_MSG);
      });
    });
    describe("advanced", async () => {
      it("minimum stake amount, in more than one transaction", async () => {
        ...
        // 2nd stake
        await consensus.stake({ from: firstCandidate, value: ONE_ETHER }).should
          .be.fulfilled;
        MIN_STAKE.should.be.bignumber.equal(
          await web3.eth.getBalance(consensus.address)
        );
      ...
      it("more than minimum stake amount, in more than one transaction", async () => {
        ...
        // 2nd stake
        await consensus.stake({ from: firstCandidate, value: TWO_ETHER }).should
          .be.fulfilled;
        MORE_THAN_MIN_STAKE.should.be.bignumber.equal(
          await web3.eth.getBalance(consensus.address)
        );
      ...
      it("more than one validator", async () => {
        ...
        pendingValidators.length.should.be.equal(1);
        // add 2nd validator
        await consensus.stake({ from: secondCandidate, value: MIN_STAKE })
          .should.be.fulfilled;
        MIN_STAKE.should.be.bignumber.equal(
          await consensus.stakeAmount(secondCandidate)
        );
  ...
  describe("delegate", async () => {
    ...
    describe("basic", async () => {
      it("should not allow zero stake", async () => {
        await consensus
          .delegate(firstCandidate, { from: firstDelegator, value: 0 })
          .should.be.rejectedWith(ERROR_MSG);
      });
      ...
      it("less than minimum stake - should not be added to pending validators", async () => {
        ...
        LESS_THAN_MIN_STAKE.should.be.bignumber.equal(
          await consensus.delegatedAmount(firstDelegator, firstCandidate)
        );
        ZERO.should.be.bignumber.equal(await consensus.totalStakeAmount());
        // pending validators should not be updated
        let pendingValidators = await consensus.pendingValidators();
        ...
        firstDelegator.should.be.equal(
          await consensus.delegatorsAtPosition(firstCandidate, 0)
        );
      });
      ...
      it("should not allow more the maximum stake", async () => {
        await consensus
          .delegate(firstCandidate, {
            from: firstDelegator,
            value: MORE_THAN_MAX_STAKE,
          })
          .should.be.rejectedWith(ERROR_MSG);
      });
    });
    describe("advanced", async () => {
      ...
      it("more than one validator", async () => {
        ...
        delegatorsLength.should.be.bignumber.equal(toBN(1));
        firstDelegator.should.be.equal(
          await consensus.delegatorsAtPosition(secondCandidate, 0)
        );
  ...
  describe("cycles and snapshots", async () => {
    ...
    it("cycle function should only be called by BlockReward", async () => {
      await consensus.cycle(initialValidator).should.be.rejectedWith(ERROR_MSG);
      await proxyStorage.setBlockRewardMock(owner);
      await consensus.cycle(initialValidator).should.be.fulfilled;
    });
  });
  ...
  describe("withdraw", async () => {
    ...
    describe("stakers", async () => {
      it("cannot withdraw zero", async () => {
        await consensus.methods["withdraw(uint256)"](ZERO_AMOUNT, {
          from: firstCandidate,
        }).should.be.rejectedWith(ERROR_MSG);
      });
      it("cannot withdraw more than staked amount", async () => {
        ...
        await consensus.methods["withdraw(uint256)"](
          MORE_THAN_MIN_STAKE
        ).should.be.rejectedWith(ERROR_MSG);
      });
      ...
      describe("stakers are not curent validators", () => {
        it("can withdraw all staked amount", async () => {
          ...
          // await mockEoC()
          // withdraw
          await consensus.methods["withdraw(uint256)"](MIN_STAKE, {
            from: firstCandidate,
          });
          MIN_STAKE.should.be.bignumber.equal(
            await web3.eth.getBalance(consensus.address)
          );
          ZERO_AMOUNT.should.be.bignumber.equal(
            await consensus.stakeAmount(firstCandidate)
          );
          // MIN_STAKE.should.be.bignumber.equal(await consensus.totalStakeAmount())
        ...
        it("can withdraw less than staked amount", async () => {
          ...
          // withdraw
          await consensus.methods["withdraw(uint256)"](ONE_ETHER, {
            from: firstCandidate,
          });
          LESS_THAN_MIN_STAKE.should.be.bignumber.equal(
            await consensus.stakeAmount(firstCandidate)
          ...
          let expectedValidators = [];
          expectedAmount.should.be.bignumber.equal(
            await web3.eth.getBalance(consensus.address)
          );
          ZERO.should.be.bignumber.equal(await consensus.totalStakeAmount());
          // pendingValidators should be updated
      ...
      describe("stakers are curent validators", () => {
        it("cannot withdraw the min stake for active validator", async () => {
          ...
          await mockEoC();
          // withdraw
          const firstCandidateBalance = toBN(
            await web3.eth.getBalance(firstCandidate)
          );
          await consensus.methods["withdraw(uint256)"](MIN_STAKE, {
            from: firstCandidate,
          }).should.be.fulfilled;
          // firstCandidate doesn't get the stake back
          firstCandidateBalance.should.be.bignumber.above(
            toBN(await web3.eth.getBalance(firstCandidate))
          );
          MIN_STAKE.should.be.bignumber.equal(
            await web3.eth.getBalance(consensus.address)
          );
        ...
        it("cannot withdraw one wei from the min stake for active validator", async () => {
          ...
          await consensus.methods["withdraw(uint256)"](ONE_WEI, {
            from: firstCandidate,
          }).should.be.fulfilled;
          // firstCandidate doesn't get the stake back
          firstCandidateBalance.should.be.bignumber.above(
        ...
        it("can withdraw until the min stake for active validator", async () => {
          ...
          MIN_STAKE.should.be.bignumber.not.equal(
            toBN(await web3.eth.getBalance(firstCandidate))
          );
          MIN_STAKE.should.be.bignumber.equal(
            await web3.eth.getBalance(consensus.address)
        ...
        it("can withdraw multiple times", async () => {
          ...
          stake.should.be.bignumber.equal(
            await web3.eth.getBalance(consensus.address)
          );
    ...
    describe("delegators", async () => {
      it("cannot withdraw zero", async () => {
        await consensus.methods["withdraw(address,uint256)"](
          firstCandidate,
          ZERO_AMOUNT,
          { from: firstDelegator }
        ).should.be.rejectedWith(ERROR_MSG);
      ...
      it("can withdraw all staked amount", async () => {
        ...
        MIN_STAKE.should.be.bignumber.equal(
          await consensus.delegatedAmount(firstDelegator, secondCandidate)
        );
        // pendingValidators should be updated
        let pendingValidators = await consensus.pendingValidators();
        pendingValidators.length.should.be.equal(1);
  ...
  describe("setValidatorFee", async () => {
    ...
    it("should only be called by validator", async () => {
      ...
      await consensus.setValidatorFee(newValidatorFee, {
        from: initialValidator,
      }).should.be.fulfilled;

      newValidatorFee.should.be.bignumber.equal(
        await consensus.validatorFee(initialValidator)
      );
      await consensus
        .setValidatorFee(newValidatorFee, { from: secondCandidate })
        .should.be.rejectedWith(ERROR_MSG);
    });
    ...
    it("should only be able to set a valid fee", async () => {
      ...
      let i;
      for (i = 15; i <= 100; i++) {
        await consensus.setValidatorFee(toBN((i / 100) * decimals), {
          from: initialValidator,
        }).should.be.fulfilled;
      }
      await consensus
        .setValidatorFee(toBN((i / 100) * decimals), { from: initialValidator })
        .should.be.rejectedWith(ERROR_MSG);
    });
    ...
    it("should not allow set less than a minimum", async () => {
      ...
      for (i = 0; i <= 14; i++) {
        await consensus
          .setValidatorFee(toBN((i / 100) * decimals), {
            from: initialValidator,
          })
          .should.be.rejectedWith(ERROR_MSG);
      }
    ...
    it("should update the fee to minimum if less than than on end of the cycle", async () => {
      ...
      await consensus.setValidatorFee(secondCandidateFee, {
        from: secondCandidate,
      }).should.be.fulfilled;
      await consensus.setValidatorFeeMock(thirdCandidateFee, {
        from: thirdCandidate,
      }).should.be.fulfilled;
      ...
      secondCandidateFee.should.be.bignumber.equal(
        await consensus.validatorFee(secondCandidate)
      );
      thirdCandidateFee.should.be.bignumber.equal(
        await consensus.validatorFee(thirdCandidate)
      );
  ...
  describe("getDelegatorsForRewardDistribution", async () => {
    ...
    it("validator without delegators", async () => {
      ...
      let { 0: delegators, 1: rewards } =
        await consensus.getDelegatorsForRewardDistribution(
          firstCandidate,
          blockRewardAmount
        );
      delegators.length.should.be.equal(0);
    ...
    describe("validator with one delegator", async () => {
      it("total delegated more than total staked - no fee", async () => {
        ...
        await consensus.setValidatorFeeMock(fee, { from: firstCandidate })
          .should.be.fulfilled;
        ...
        let expectedReward = blockRewardAmount
          .mul(delegateAmount)
          .div(MIN_STAKE)
          .mul(decimals.sub(validatorFee))
          .div(decimals);
    ...
    describe("validator with multiple delegators", async () => {
      it("total delegated more than total staked - no fee", async () => {
        ...
        await consensus.delegate(firstCandidate, {
          from: firstDelegator,
          value: firstDelegateAmount,
        }).should.be.fulfilled;
        await consensus.delegate(firstCandidate, {
          from: secondDelegator,
          value: secondDelegateAmount,
        }).should.be.fulfilled;
        await consensus.setValidatorFeeMock(fee, { from: firstCandidate })
          .should.be.fulfilled;
        let validatorFee = await consensus.validatorFee(firstCandidate);
        ...
        rewards.length.should.be.equal(delegators.length);
        rewards[0].should.be.bignumber.equal(
          blockRewardAmount
            .mul(firstDelegateAmount)
            .div(MIN_STAKE)
            .mul(decimals.sub(validatorFee))
            .div(decimals)
        );
        rewards[1].should.be.bignumber.equal(
          blockRewardAmount
            .mul(secondDelegateAmount)
            .div(MIN_STAKE)
            .mul(decimals.sub(validatorFee))
            .div(decimals)
        );
    ...
    it("validator with many delegators", async () => {
      ...
      let delegateAmountValue = parseInt(
        (MIN_STAKE_AMOUNT * 0.99) / delegatorsCount
      );
      let delegateAmount = toWei(toBN(delegateAmountValue), "ether");
      let stakeAmountValue =
        MIN_STAKE_AMOUNT - delegateAmountValue * delegatorsCount;
      let stakeAmount = toWei(toBN(stakeAmountValue), "ether");
      let fee = toBN(0.05 * decimals);
      let validator = accounts[1];
      await consensus.sendTransaction({ from: validator, value: stakeAmount })
        .should.be.fulfilled;
      for (let i = 2; i < accounts.length; i++) {
        await consensus.delegate(validator, {
          from: accounts[i],
          value: delegateAmount,
        }).should.be.fulfilled;
      }
      await consensus.setValidatorFeeMock(fee, { from: firstCandidate }).should
        .be.fulfilled;
      let validatorFee = await consensus.validatorFee(firstCandidate);
      let { 0: delegators, 1: rewards } =
        await consensus.getDelegatorsForRewardDistribution(
  ...
  describe("Jailing", async () => {
    ...
    it("Validator should not be jailed twice when inactive for two cycles", async () => {
      await consensus.sendTransaction({
        from: firstCandidate,
        value: MIN_STAKE,
      }).should.be.fulfilled;
      ...
      ONE.should.be.bignumber.equal(await consensus.jailedValidatorsLength());
    ...
    it("Check jailing threshold", async () => {
      ...
      currentValidators.length.should.be.equal(2);
      let expected =
        (CYCLE_DURATION_BLOCKS * JAIL_CUTOFF) / currentValidators.length + 1;
      await consensus.setBlockCounterMock(firstCandidate, expected);
      await consensus.setBlockCounterMock(secondCandidate, expected);

      toBN(expected).should.be.bignumber.equal(
        await consensus.blockCounter(firstCandidate)
      );
      toBN(expected).should.be.bignumber.equal(
        await consensus.blockCounter(secondCandidate)
      );
      ...
      false.should.be.equal(
        await consensus.isPendingValidator(secondCandidate)
      );
    });
    ...
    it("Check strikes", async () => {
      ...
      for (i = 3; i <= 5; i++) {
        ...
        blocksToAdvance =
          currentCycleEndBlock.toNumber() - currentBlockNumber + 1;
        ...
        true.should.be.equal(await consensus.isJailed(firstCandidate));
        false.should.be.equal(
          await consensus.isPendingValidator(firstCandidate)
        );
        await consensus.unJail({ from: firstCandidate }).should.be.fulfilled;
        false.should.be.equal(await consensus.isJailed(firstCandidate));
        ...
        await blockReward.cycleMock({ from: owner }).should.be.fulfilled;
        if (i < 5) {
          toBN(i).should.be.bignumber.equal(
            await consensus.getStrikes(firstCandidate)
          );
        } else {
          toBN(5).should.be.bignumber.equal(
            await consensus.getStrikes(firstCandidate)
          );
        }
      }
  ...
  describe("Maintenance", async () => {
    ...
    it("Only validators can flag maintenance", async () => {
      //test with non val
      await consensus
        .maintenance({ from: firstCandidate })
        .should.be.rejectedWith(ERROR_MSG);
  ...
  describe("upgradeTo", async () => {
    ...
    it("should only be called by ProxyStorage", async () => {
      ...
      await proxy
        .upgradeTo(consensusNew.address, { from: owner })
        .should.be.rejectedWith(ERROR_MSG);
      let { logs } = await proxy.upgradeTo(consensusNew.address, {
        from: proxyStorageStub,
      });
      logs[0].event.should.be.equal("Upgraded");
      await proxy.setProxyStorageMock(proxyStorage.address);
    });
    it("should change implementation address", async () => {
      consensusOldImplementation.should.be.equal(
